...
Статус документа: это концепт и архитектурное обоснование. Текущий краткий статус,
команды запуска и проверок находятся в [README.md](README.md), а GPU/WebGL-трек
описан в [docs/gpu-webgl-roadmap.md](docs/gpu-webgl-roadmap.md). Исторические
этапы ниже оставлены как объяснение выбранного пути, а не как актуальный список
...
5. Запустить Chromium как единственный UI через минимальный Wayland compositor.
...
init / supervisor
        ...
        |       +-- localhost-only HTTP gateway for the browser
...
Текущий репозиторий уже выбрал отдельную initramfs/QEMU-сборку поверх Alpine package source: это ближе к appliance OS, чем к установленному дистрибутиву в UTM. Buildroot или другой контролируемый build pipeline остаются возможным следующим этапом, когда текущий SuvOS layer стабилизируется. Yocto имеет смысл рассматривать только при промышленной сборочной инфраструктуре и более широкой матрице устройств.
...
- Linux VM с x86_64 target path и arm64/HVF dev path на Apple Silicon;
- пользователь `suvos`;
...
- есть bootable Linux-based прототип: x86_64 QEMU/TCG compatibility path и быстрый `aarch64` QEMU-HVF path на Apple Silicon;
...
- есть фоновый `suvosd`, собранный как статический C++ binary под целевую архитектуру;
...
- `suvosctl` проверяет Unix socket API и служит диагностическим клиентом для control API;
- `suvos-gateway` слушает loopback-only `127.0.0.1:80`, а guest `/etc/hosts` мапит `suv.os` на `127.0.0.1`;
- текущая web UI лежит в `/system/suvos/ui` и открывается через `http://suv.os/`;
...
- `suvos.gui=1` режим запускает browser shell через `suvos-start-gui`: Cage + обычный Chromium;
...
- `/data/suvos` создается как writable tmpfs-зона для runtime state, browser state, AEC state и будущих пользовательских расширений;
...
- `SuvOS_AEC` и `SuvOS_Chromium` подключены как отдельные vendor checkout'ы через `third_party/vendors.lock.json`;
- обычный `make run` поднимает GUI+AEC профиль и открывает вкладки `http://suv.os/` и `http://suv.os/aec/`.
...
make run-qemu-x86 # x86_64 QEMU/TCG compatibility backend
...
- notification feed, power UX, системная панель, browser-chrome controls и OS-facing settings должны переехать в `SuvOS_Chromium` patchset или privileged internal page.
...
Admin Explorer Code (AEC) - это отдельная admin/debug web-вкладка внутри Chromium. AEC является root-capable guest filesystem explorer: файловое дерево, вкладки, просмотр текстов/логов/картинок и встроенный terminal работают по гостевой SuvOS filesystem, включая root-level доступ внутри VM. Host filesystem этот профиль не затрагивает.

Это осознанное исключение из будущей policy-aware пользовательской file API модели. Риск ограничивается profile-level включением AEC, запуском только внутри guest VM и сохранением `/system/suvos` read-only boot policy. Обычный `make run` включает GUI+AEC для разработки; core/full тесты и будущие пользовательские file APIs не должны зависеть от AEC. Если понадобится запись в `/system/suvos` из AEC, это должно быть отдельным явным изменением boot policy.

Текущая модель AEC:

1. AEC живет как отдельный fork `SuvOS_AEC`; исходники Code - OSS не клонируются в SuvOS repo.
2. Checkout подтягивается через `third_party/vendors.lock.json` + `scripts/bootstrap-vendors.sh`.
3. AEC artifact собирается как Code - OSS source fork без marketplace gallery, Microsoft/cloud endpoints, telemetry defaults, settings sync, account auth defaults и chat/Copilot contributions.
4. Для ручного запуска AEC включен через `make run` / `make runos`; низкоуровневые `SUVOS_WITH_AEC=1`, `make run-gui-aec` и `make test-aec-smoke` оставлены для совместимости и автотестов.
5. AEC server внутри guest работает как root на `127.0.0.1:3030` с base path `/aec`.
6. Runtime state хранится в `/data/suvos/aec`, local extensions - в `/data/suvos/extensions/aec`, volatile state - в `/run/suvos-aec`.
7. Smoke-тест проверяет workbench, static assets, отсутствие marketplace/cloud product metadata и работоспособность `node-pty` для terminal.
8. В default QEMU GUI-профиле terminal rendering остается software-only. WebGL для xterm можно возвращать только отдельным accelerated VM треком и отдельным smoke для Chromium WebGL/xterm WebGL.
...
Chromium-трек уже вынесен в отдельный vendor repo `SuvOS_Chromium`, который SuvOS подключает через `third_party/vendors.lock.json` и browser artifact. Текущая реализация этого repo пакует отдельный Chromium rootfs overlay на базе Alpine `chromium` package. Более глубокий source-level patchset в `SuvOS_Chromium` нужен только после того, как этот vendor path, обычный Chromium под Cage, web UI, `suvos-gateway`, file API и XDG portal path перестанут покрывать требования. Следующий Chromium-трек должен идти так:

1. Держать отдельный checkout/repo для Chromium, не внутри SuvOS.
2. Сначала сохранить рабочий vanilla/vendor artifact path без SuvOS patchset.
3. Затем добавить один минимальный patch и отдельную patch queue.
...
5. После этого заменить текущий packaged-browser artifact на source-built Chromium artifact в том же vendor repo.
...
Chromium большой. Собирать его из исходников долго и тяжело. В SuvOS уже есть отдельный vendor repo `SuvOS_Chromium`; текущий artifact path пакует browser rootfs overlay на базе Alpine `chromium`. Глубокий source-level patchset должен развиваться в этом vendor repo, когда обычный Chromium под Cage, policies/profile configuration, `suvos-gateway`, XDG portal route и web UI перестанут покрывать требования.

Официальный Linux build path Chromium рассчитан на x86-64 machine, минимум 8GB RAM, желательно больше 16GB, и как минимум 100GB свободного места. На Mac M для целевого x86_64 source build это лучше рассматривать как отдельную Linux x86_64 build-машину или удаленный build host, а не как часть обычной SuvOS initramfs-сборки.
...
3. File API v1: `/api/files/*` через `suvos-gateway -> suvosd` для будущего policy-aware пользовательского file picker / file manager трека.
4. AEC hardening: держать `SuvOS_AEC` artifact проверяемым smoke-тестом, не возвращать marketplace/cloud/chat defaults и не связывать его с пользовательским file API.
5. Chromium integration: проверить XDG portal route для file chooser и затем проектировать patchset в `SuvOS_Chromium` как место для OS-facing settings, power/shutdown UX и browser-chrome integration.
...
Следующие коммиты должны оставаться маленькими и проверяемыми: session token, File API, AEC hardening или узкий Chromium integration patch. Исходники Code - OSS и Chromium не должны попадать в текущий SuvOS repo.
...
- AEC включается profile-level флагом `SUVOS_WITH_AEC=1`; обычный manual `make run` использует GUI+AEC профиль;
