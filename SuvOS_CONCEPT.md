...
init / supervisor
        ...
        |
        +-- local API gateway
        |       +-- Unix socket or localhost-only HTTP/gRPC
...
- `suvosd run` запускает только приложения из `/system/suvos/apps/registry.tsv`, а имена с `/` и `..` блокируются;
...
- `suvosd` проверяет app registry и capability-политику;
...
Для IPC на старте лучше использовать Unix domain socket. HTTP на `127.0.0.1` можно использовать позже для браузерного UI, но важно не открыть API наружу.
...
1. Скачать UTM.
2. Создать x86_64 Linux VM, если целимся в обычный PC VM формат.
3. Поставить минимальный Linux без desktop environment.
4. Создать `/system/suvos`.
5. Написать первый `suvos` CLI.
6. Добавить allowlist запуска скриптов.
7. Проверить `suvos run hello`.
8. Затем заменить прямой запуск скриптов на `suvosd`.

После этого уже имеет смысл переходить к Buildroot и собирать первый маленький bootable image.
