...
Large upstream forks are not stored inside this repository. Instead of submodules, SuvOS uses `third_party/vendors.lock.json` plus `make bootstrap-vendors`. There are now two required vendor checkouts: `SuvOS_AEC` under `third_party/aec` and `SuvOS_Chromium` under `third_party/chromium`. AEC supplies the editor artifact, while `SuvOS_Chromium` supplies the browser rootfs overlay artifact from a pinned ref.
